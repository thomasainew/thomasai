...
const DEFAULTS: Record<AdvisorPersona['id'], { name: string; emoji: string; color: string; placeholder: string }> = {
  achachan: {
    name: 'അച്ചച്ചൻ',
    emoji: '👴',
    color: '#3b82f6',
    placeholder: 'e.g. "He always brings up how he saved for his own house." or "Speak a bit more strictly about loans."',
  },
  chachan: {
    name: 'ചാച്ചൻ',
    emoji: '👨‍💼',
    color: '#10b981',
    placeholder: 'e.g. "He is a bank manager, so he loves comparing interest rates." or "Keep his tone lighter, less scolding."',
  },
}
...
function PersonaCard({ id }: { id: AdvisorPersona['id'] }) {
  ...
  const save = () => {
    upsertAdvisorPersona({ id, name: name.trim() || base.name, photo: photo || undefined, instructions: instructions.trim() || undefined })
    setSavedFlash(true)
  ...
  return (
    <div className="card p-5">
      ...
      <input className="input mb-3" value={name} onChange={(e) => setName(e.target.value)} placeholder={base.name} />

      <label className="label">Personality &amp; training notes</label>
      <textarea
      ...
      <p className="text-[11px] text-slate-400 mt-1.5 leading-relaxed">
        This is not model fine-tuning — it's added to {name}'s prompt every time they reply, so describe how they
        should sound: tone, background, catchphrases, things to emphasise or avoid.
      </p>
  ...
  return (
    <div className="space-y-5 max-w-[1100px]">
      <PageHeader
        ...
        subtitle="Give Achachan and Chachan a photo and a personality — it shapes how they reply, in Malayalam, to your real figures."
        actions={
